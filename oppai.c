...
int pp_std(ezpp_t ez) {
  ...
  float length_bonus = (
    ...
    float combo_break = (
    (float)pow(ez->combo, 0.2f) / (float)pow(ez->max_combo, 0.2f)
  );
  float miss_penality = ez->mods & MODS_RX
    ? (float)pow(0.97f, (ez->nmiss * 1.25 + ez->n50 * 0.35f) * combo_break) * combo_break
    : (float)(pow(0.97f, ez->nmiss) * combo_break) * combo_break;
  float ar_bonus;
  ...
  if (ez->nmiss >= 1) {ez->aim_pp *= miss_penality;}
  ez->aim_pp *= ar_bonus;
  ...
  if (ez->mods & MODS_RX) {
    ...
    if (ez->mods & MODS_HR) {
      ez->aim_pp *= pow((ez->cs / 2), 0.08);;
      ez->speed_pp *= pow((ez->od /2), 0.08);;
    }
  }}
  ...
  if (ez->n50 > (ez->nobjects / 500.0f)) {
    ez->speed_pp *= (float)pow(0.83, (ez->n50 - (ez->nobjects / 500.0f)));
  }
  // nerf fc's because fuck sunglow amiright :clueless: :tf:
  /*if (ez->nmiss == 0) {
    ez->aim_pp *= (float)al_min(ez->aim_stars, 1.06);
  }
  else {
      ez->aim_pp *= (float)al_min(ez->aim_stars, 1.17);
  }*/
  ...
  acc_sadge = accuracy * 50.0f;
  if (ez->accuracy_percent < 95.03) ez->pp -= acc_sadge;
  if (ez->pp < 0) ez->pp = 0;
  if (ez->mods & MODS_RX & MODS_DT & MODS_HR) ez->pp *= (1.017 / (ez->nmiss + 1));
...
int pp_taiko(ezpp_t ez) {
  ...
  /* miss penality */
  ez->speed_pp *= (float)pow(1.023f, ez->nmiss);
